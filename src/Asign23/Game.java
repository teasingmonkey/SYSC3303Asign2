...
public static GameServe g;
	...
	public void addWord(Socket p1, String wordP1)
	{
		for(GameStart gse: totalGames)
		{
			if(gse.s1.equals(p1))
			{
				...
				gse.incrementPlayer1Counter();
			}
			if(gse.s2.equals(p1))
			{
				gse.player2Words.add(wordP1);
				gse.incrementPlayer2Counter();
			}
		}
	}
	
	public String getWords(Socket p1, int i)
	{
		String s = "";
		if(totalGames.get(i).s1.equals(p1))
		{
			for(int j=0; j< totalGames.get(i).player1Words.size();j++)
				{
					s = s + totalGames.get(i).player1Words.toString() + "\n";
				}
			return s;
		}
		if(totalGames.get(i).s2.equals(p1))
		{
			for(int j=0; j< totalGames.get(i).player2Words.size();j++)
			{
				s = s + totalGames.get(i).player2Words.toString() + "\n";
			}
		return s;
		}
		return s;
	...
	public String getScoreForFile(Socket p1, int i)
	{
		String s = "";
		if(totalGames.get(i).s1.equals(p1))
		{
			return s = ""+totalGames.get(i).player1Words.size();
		}
		if(totalGames.get(i).s2.equals(p1))
		{
			s = ""+totalGames.get(i).player2Words.size();
		}
		return s;
	}
	
	public void emptyBuffer(Socket p1, int i)
	{
		if(totalGames.get(i).s1.equals(p1))
		{
			totalGames.get(i).emptyPlayer1Buffer();
			totalGames.get(i).sortPlayer1();
		}
		if(totalGames.get(i).s2.equals(p1))
		{
			totalGames.get(i).emptyPlayer2Buffer();
			totalGames.get(i).sortPlayer2();
		}
	}
	...
	public void getScore(Socket p1, int i) throws IOException
	{
		if(totalGames.get(i).s1.equals(p1))
		{
			g.sendMessage("Your Score : " + totalGames.get(i).getPlayer1Score(), p1);
		}
		if(totalGames.get(i).s2.equals(p1))
		{
			g.sendMessage("Your Score : " + totalGames.get(i).getPlayer2Score(), p1);
		}
	}
}
